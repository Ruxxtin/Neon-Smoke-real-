...
func _on_scene_changed(scene_path):
	if scene_path == "res://scenes/arcade_game_1.tscn" or scene_path == "res://scenes/arcade_game_1_internal.tscn":
		if not is_playing():
			play()
			print("Playing audio for Arcade Game 1!")
	else:
		if is_playing():
			stop()
			print("Stopped audio as the scene changed.")
