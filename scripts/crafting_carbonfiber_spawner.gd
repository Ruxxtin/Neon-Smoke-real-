...
var carbonfiber_amount = ResourceManager.carbonfiber_amount
...
func _process(delta):
	carbonfiber_amount = ResourceManager.carbonfiber_amount
	label.text = str(carbonfiber_amount)
	
	if Input.is_action_just_pressed("click") and carbonfiber_amount >= 1 and touching_mouse == true:
		if able_to_spawn:
			spawn()
